...
public class Server implements Closeable
{
	...
	private class ServerManager extends Thread implements Closeable
	{
			...
			try{//wrapper for the server manager loop
				...
				while(keepRunning){//while the server manager should continue running
					...
					/*TODO
						...
						synchronized(clientQueues){//lock clientQueues
							clientQueues.push(clientManager);//add the new client manager at the front of the queue list
						}//release clientQueues
					TODO*/
				...
				synchronized(serverInput){//sync serverInput, keepRunning, parent
					...
					if(keepRunning){//if the server manager thread should still be running
						if(parent != null){//if this server manager was a reconstruction attempt
							...
							try{//try to reconstruct the server manager thread to replace this one
								ServerManager child = new ServerManager(serverInput.getLocalPort(), serverOutput.getLocalPort());//create a new server manager
		...
		{
			synchronized(serverInput){//lock serverInput, keepRunning
			...
			}//release serverInput, keepRunning
		}
	}
}
